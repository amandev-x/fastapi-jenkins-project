...
def create_todo_empty_title():
    response = client.post(
        "/todos",
        json={"title": " ", "description": "Test empty title", "completed": False},
    )
...
def test_update_todo():
    ...
    # Update it
    response = client.put(
        f"/todos/{todo_id}", json={"title": "New Title", "completed": True}
    )
    assert response.status_code == 200
...
def test_delete_todo():
    # Create a todo
    response = client.post(
        "/todos", json={"title": "To be deleted", "completed": False}
    )
    assert response.status_code == 201
    todo_id = response.json()["id"]
